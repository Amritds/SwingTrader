...
### Outputs
Activity of the trading-bot is continuously printed to stdout. It is recommended that this code be run in a separate terminal with tmux to monitor output messages. Current stock holdings, quantities owned, current stocks tracked and those marked for purchase and sale are periodically dumped to pickle files ```holdings.pkl, quantity.pkl, current_track.pkl, cool_track.pkl, hot_track.pkl``` to allow seamless recovery if the bot crashes.
